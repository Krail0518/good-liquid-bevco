...
--   -- 'can_cost_per_unit' is NOT restored: it was superseded, not moved, and
--   -- nothing reads it.
--
-- ALREADY APPLIED. These four migrations were run against production on
-- 2026-08-24 but never recorded in supabase_migrations.schema_migrations, so
-- the history did not show them. All 14 keys they create are present and were
-- confirmed before this file was committed. Re-running is a no-op: the inserts
-- are ON CONFLICT DO NOTHING and the deletes target keys that no longer exist.
--
-- One consequence worth knowing: keg_minimum is 50 in production, not the 40
-- inserted here. ON CONFLICT DO NOTHING means the live value wins, which is the
-- intended behaviour — a migration should not overwrite a price somebody set.
