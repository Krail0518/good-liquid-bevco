-- ROLLBACK:
--   None that is meaningful. This sets placeholder prices to 0 deliberately, so
--   restoring the placeholders would put invented numbers back into quotes.
--   The correct reversal is for staff to enter the real prices in the editor.
--
-- ALREADY APPLIED (see the note in 20260824150000).
--
-- WHY ZERO RATHER THAN A GUESS
-- A placeholder price that looks plausible is worse than one that is obviously
-- unset: it gets quoted to a customer and nobody notices. Zero shows up
-- immediately in a quote total, which is the point.
