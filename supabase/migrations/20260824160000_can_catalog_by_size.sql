-- ROLLBACK:
--   delete from public.pricing_settings where key like 'can_blank_%_per_unit'
--      or key like 'can_shrink_%_per_unit' or key like 'can_printed_%_per_unit';
--   -- then re-run 20260824150000 to restore the three generic can keys.
--
-- ALREADY APPLIED (see the note in 20260824150000). Splits the three generic
-- can prices into eight size-specific ones, because a 12oz sleeve and a 19.2oz
-- sleeve are not the same cost and quoting them as one was wrong.
