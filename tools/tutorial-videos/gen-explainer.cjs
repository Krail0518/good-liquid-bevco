 ...
 * decks: gmp-haccp-pcqi | lacf | sqf | allergen | envmon | sanitation |
 *        traceability | deal-brief | deal-docs
 */
...
'envmon':{ brand:'Environmental Monitoring', slides:[
  {say:"In this video we'll cover environmental monitoring — the program that hunts for dangerous microbes in your plant, before they ever reach your product.",html:titleSlide('ENVIRONMENTAL MONITORING','Finding pathogens before they find your product','Your early-warning system for the plant')},
  {say:"Here's the key idea. Pathogens like Listeria don't just arrive on ingredients — they live and hide in the plant environment itself: in drains, on floors, in the nooks of equipment. Environmental monitoring is how you go looking for them on purpose, instead of waiting to find them in a finished product.",html:bulletsSlide('Why monitor the environment?',['Pathogens like <b>Listeria</b> live in drains, floors &amp; equipment','They can transfer to product during production','You find them <b>on purpose</b> — proactively','It’s an early-warning system, not a final test'],RED)},
  {say:"You do it by swabbing surfaces around the plant and sending the swabs for testing. The magic is in where you swab — organized into four zones, based on how close each surface is to the food.",html:sectionCard('🧫','How it works','Swab the plant in zones, then test',K)},
  {say:"Zone one is food-contact surfaces — fillers, seamers, anything the product touches. Zone two is right next to them — equipment framework and nearby surfaces. Zone three is further out — floors, drains, and walls in the processing area. And zone four is outside processing altogether — warehouses, hallways, and locker rooms. A hit in zone one is the most urgent.",html:principlesSlide('The four zones',BLUE,[['1','Food-contact surfaces'],['2','Near food-contact'],['3','Floors, drains &amp; walls'],['4','Outside processing']])},
  {say:"What are you testing for? Usually Listeria — both the broad Listeria species, as an indicator that conditions are right, and Listeria monocytogenes, the dangerous one. Depending on your product you might also test for Salmonella, or for general indicators like total plate count and coliforms.",html:bulletsSlide('What you test for',['<b>Listeria species</b> — an indicator organism','<b>L. monocytogenes</b> — the pathogen itself','Sometimes <b>Salmonella</b>, by product risk','Indicators like total plate count &amp; coliforms'],AMBER)},
  {say:"The most important part isn't the swab — it's what you do when one comes back positive. This is called seek-and-destroy: you investigate, intensify cleaning, and re-swab a wider area to find the source and confirm it's gone.",html:sectionCard('🚨','A positive result','Seek and destroy — find the source, prove it’s gone',RED)},
  {say:"When a zone one or two surface comes back positive, you stop and respond. Assess any product that was exposed. Deep-clean and sanitize the area. Expand your swabbing outward to find where it's coming from — and keep re-swabbing until the results are clean again.",html:bulletsSlide('The seek-and-destroy response',['Assess &amp; <b>hold</b> any exposed product','<b>Deep-clean</b> and sanitize the area','<b>Expand swabbing</b> to find the source','Re-swab until results are clean'],RED)},
  {say:"And you track it over time. One positive is an event; a pattern of positives in the same spot is a warning that something systemic is wrong. Trending your results turns raw swabs into real insight.",html:titleSlide('Trend it','One positive is an event','A pattern is a warning — trending turns swabs into insight')},
  {say:"So the cycle is simple: swab your zones on a schedule, test them, trend the results, and when something's positive, seek and destroy. Repeat, always.",html:flowSlide('The monitoring cycle',[['🧫','Swab','on schedule',K],['🔬','Test','Listeria &amp; more',AMBER],['📈','Trend','watch for patterns',BLUE],['🚨','Act','seek &amp; destroy',RED]],'A living cycle — the plant is never “done” being monitored.')},
  {say:"Done well, environmental monitoring is how you prove your sanitation actually works, and how you catch a problem while it's still on a drain — not in a consumer's hand. It's one of the strongest signals that your food-safety system is real.",html:closeSlide('The bottom line','Catch it on the drain, <span style="color:'+K+'">not</span> in the product',[['🧫 Swab the zones',K],['📈 Trend the results',BLUE],['🚨 Seek &amp; destroy',RED]],'Educational overview — not legal advice. Design your EMP with a qualified food-safety professional.')}
]},

'sanitation':{ brand:'Sanitation & CIP', slides:[
  {say:"In this video we'll cover sanitation — the single most important program in any food plant — and C I P, the automated way to clean the equipment you can't take apart.",html:titleSlide('SANITATION & CIP','Clean first, then sanitize','The program everything else rests on')},
  {say:"Why start here? Because sanitation is the foundation of good manufacturing practice, and the majority of food recalls trace back to environmental contamination. If the plant isn't clean, no plan can make the product safe.",html:bulletsSlide('Why sanitation comes first',['It’s the <b>#1 GMP program</b>','Most recalls trace to <b>environmental contamination</b>','A clean plant is the base for everything else','Auditors look here first'],RED)},
  {say:"First, a distinction people get wrong all the time. Cleaning and sanitizing are two different jobs. Cleaning removes the visible soil — the residue and buildup. Sanitizing kills the microbes you can't see. And the order matters: you must clean first, because sanitizer can't work through a layer of soil.",html:compareSlide('Clean ≠ sanitize',[
    {label:'Cleaning',color:BLUE,lines:['Removes <b>visible soil</b> &amp; residue','Detergent + water + scrubbing','Must come <b>first</b>']},
    {label:'Sanitizing',color:K,lines:['<b>Kills microbes</b> you can’t see','A chemical or heat step','Only works on a <b>clean</b> surface']}
  ])},
  {say:"For open surfaces, wet cleaning follows a set sequence: dry pick-up of loose debris, a pre-rinse, washing with detergent, a rinse, then sanitizing — and a final rinse if the sanitizer requires it. Each step has a purpose, and skipping one undermines the rest.",html:principlesSlide('The wet-cleaning steps',BLUE,[['1','Dry pick-up'],['2','Pre-rinse'],['3','Wash (detergent)'],['4','Rinse'],['5','Sanitize'],['6','Verify']])},
  {say:"For tanks, pipes, and fillers you can't disassemble, you use Clean In Place — C I P. The equipment cleans its own interior by circulating solutions in a controlled cycle: a rinse, a caustic wash to cut soil, another rinse, often an acid wash, and a sanitizer — each controlled by time, temperature, and concentration.",html:bulletsSlide('CIP — Clean In Place',['For tanks, lines &amp; fillers you <b>can’t take apart</b>','Circulates solutions through the equipment','Cycle: rinse → <b>caustic</b> → rinse → <b>acid</b> → sanitize','Controlled by <b>time, temperature &amp; concentration</b>'],AMBER)},
  {say:"Before every startup, you verify. A pre-operational inspection confirms surfaces are visually clean and, often, checks them with an A T P swab that measures invisible residue. Nothing runs until pre-op passes.",html:bulletsSlide('Pre-operational inspection',['Done <b>before startup</b>, every run','Visual check of every food-contact surface','Often an <b>ATP swab</b> for invisible residue','A fail means re-clean &amp; re-check — no production'],K)},
  {say:"Not everything is cleaned every day. A Master Sanitation Schedule is the calendar that assigns the less-frequent tasks — walls, overheads, drains, and hard-to-reach areas — to a day and an owner, so nothing is quietly forgotten.",html:titleSlide('The Master Sanitation Schedule','Everything cleaned, on a calendar','Walls · overheads · drains · hard-to-reach — assigned &amp; tracked')},
  {say:"And you prove it works. Validation shows your cleaning method truly removes soil and microbes; verification — through A T P checks and swabs — confirms it's working day to day. Both are recorded.",html:bulletsSlide('Validation & verification',['<b>Validate</b>: prove the method removes soil &amp; microbes','<b>Verify</b>: confirm it’s working — ATP &amp; swabs','Tie it to your environmental monitoring','<b>Record</b> everything'],K)},
  {say:"So the rhythm is: clean to remove the soil, rinse it away, sanitize to kill what's left, and verify the result. Do that reliably, and you've built the foundation every other food-safety program stands on.",html:flowSlide('The sanitation rhythm',[['🧹','Clean','remove soil',BLUE],['💧','Rinse','wash it away',K],['🧴','Sanitize','kill microbes',AMBER],['✅','Verify','prove it worked',RED]],'Clean first — sanitizer can’t work through soil.')},
  {say:"Get sanitation right and everything above it gets easier. It's not glamorous, but it's the program that quietly keeps your product, and your brand, safe.",html:closeSlide('The bottom line','Clean plant, <span style="color:'+K+'">safe</span> product',[['🧹 Clean, then sanitize',BLUE],['✅ Pre-op every run',K],['🗂️ Validate &amp; record',AMBER]],'Educational overview — not legal advice. Build validated sanitation SOPs for your equipment and chemicals.')}
]},

'traceability':{ brand:'Traceability & Recall', slides:[
  {say:"In this video we'll cover traceability and recall — how you account for every unit you make, and how you get it back quickly if something ever goes wrong.",html:titleSlide('TRACEABILITY & RECALL','Account for every unit — fast','The drill every auditor tests')},
  {say:"Why does it matter? Because if there's ever a problem, the F D A expects you to identify and locate all affected product quickly — the standard is being able to do it within four hours. A recall is the ultimate test of whether your whole record system actually works.",html:bulletsSlide('Why it matters',['FDA expects you to locate affected product <b>fast</b>','The benchmark: within <b>4 hours</b>','A recall tests your <b>entire</b> record system','Speed limits the damage — to consumers and your brand'],RED)},
  {say:"It all hangs on one thing: the lot code. Every batch you produce gets a unique code, printed on the product. That code is the thread that ties an ingredient delivery to a finished can to a customer's shipment.",html:sectionCard('🏷️','The lot code','One unique code per batch — your traceability thread',K)},
  {say:"With that thread you trace in two directions. One-back: for any lot, what went into it — which supplier, which ingredient lots. One-up: where that lot went — which customers received it, and how much. Backward to the source, forward to the shelf.",html:compareSlide('Trace both directions',[
    {label:'One-back',color:BLUE,lines:['What <b>went into</b> the lot','Supplier &amp; ingredient lots','Traces to the <b>source</b>']},
    {label:'One-up',color:K,lines:['Where the lot <b>went</b>','Which customers &amp; how much','Traces to the <b>shelf</b>']}
  ])},
  {say:"You practice this before you ever need it, with a mock recall. You pick a lot and see how quickly and completely you can account for all of it — aiming to reconcile it within a couple of hours. It's a fire drill for your records.",html:bulletsSlide('The mock recall — practice',['Pick a lot and <b>account for all of it</b>','Time yourself — aim for a couple of hours','Measure the <b>% reconciled</b>','A fire drill you run <i>before</i> you need it'],AMBER)},
  {say:"If a real recall is ever needed, a written recall plan takes over. Here's the shape of it.",html:sectionCard('📋','The recall plan','What happens when it’s real',BLUE)},
  {say:"You assemble the recall team and classify the health risk — Class one is dangerous, class three is minor. You notify the affected customers, and the F D A when required. You control and retrieve the product, and decide its fate — destroy or rework. Then you verify how much you recovered, and fix the root cause so it never recurs.",html:principlesSlide('The recall steps',BLUE,[['1','Assemble team &amp; classify'],['2','Notify customers &amp; FDA'],['3','Control &amp; retrieve'],['4','Decide disposition'],['5','Verify effectiveness'],['6','Fix the root cause']])},
  {say:"Put simply, the whole flow runs on that lot code: it lets you trace the product, notify the right people, and reconcile what you recover — the difference between a contained, hours-long event and an out-of-control one.",html:flowSlide('The flow, end to end',[['🏷️','Lot code','the thread',K],['🔗','Trace','up &amp; back',BLUE],['📣','Notify','customers &amp; FDA',AMBER],['✅','Reconcile','account for it',RED]],'Every step depends on clean lot coding and complete records.')},
  {say:"The takeaway: run mock recalls regularly, so that if the real thing ever comes, it's muscle memory. Good traceability turns a potential catastrophe into a controlled, provable process.",html:closeSlide('The takeaway','Make the real recall <span style="color:'+K+'">muscle memory</span>',[['🏷️ Code every lot',K],['🔁 Run mock recalls',AMBER],['📋 Keep a recall plan',BLUE]],'Educational overview — not legal advice. Keep a written recall plan and test it regularly.')}
]},
...
'deal-brief':{ brand:'Never Drop a Lead', slides:[
  {say:"In this video we'll tour three new tools that stop good leads from slipping through the cracks — the Deal Brief, the Needs Attention board, and a morning nudge that lands right on your phone.",html:titleSlide('NEVER DROP A LEAD','Your CRM now reads your email for you','The Deal Brief · the Attention board · a morning digest')},
  {say:"Here's the problem we set out to solve. When dozens of deals are open at once and the inbox never stops, it's easy to lose track of who's waiting on you, who's gone quiet, and what you promised. Deals rarely die from being bad — they die from being forgotten.",html:bulletsSlide('The problem',['Dozens of open deals, a nonstop inbox','Easy to forget who is waiting on <b>you</b>','Easy to miss a lead going quiet','Good deals die from being <b>forgotten</b>'],RED)},
  {say:"The first tool is the Deal Brief. Open any deal or client, and at the very top you'll find a living summary that the AI keeps up to date from the entire email thread and your notes.",html:sectionCard('🧠','The Deal Brief','A living summary on every deal and client',K)},
  {say:"The brief answers the four questions you actually care about. Where does this stand? Whose court is the ball in — and for how long? What are the key facts, like volume and pricing? And what are the next actions? Those actions are checkboxes you tick off, and they stick.",html:bulletsSlide('What the brief shows',['<b>Status</b> — where the deal stands','<b>Ball in court</b> — you or them, and how long','<b>Key facts</b> — volume, format, quote, timeline','<b>Next actions</b> — checkable to-dos that stick'],K)},
  {say:"Here's the clever part. Every time a new email arrives, the brief doesn't re-read the whole history — it folds in only what's new and merges it. So it stays current, it never repeats itself, and it barely costs a thing.",html:titleSlide('The clever part','It only re-reads what is new','Folds each new email into the brief — no repeats, no waste')},
  {say:"The second tool answers a bigger question: of all my open deals, who do I touch first? That's the Needs Attention board — one ranked list across your entire pipeline.",html:sectionCard('🔥','Needs Attention','One ranked list across every open deal',AMBER)},
  {say:"It sorts every open deal into four buckets, most urgent first. Your move — they replied and you haven't. Overdue — a task you own is past due. Cold — you reached out and it went quiet for a week or more. And new — a lead with an email you've never contacted. Anything where the ball is genuinely in their court drops off the list.",html:principlesSlide('How it ranks your day',AMBER,[['🟡','Your move'],['⏰','Overdue'],['🧊','Cold'],['✨','New lead']])},
  {say:"The third tool comes to you. Every weekday morning, that same ranked list is delivered as a WhatsApp message and an email — so you start the day knowing exactly who needs you, before you even open the CRM. Quiet days stay quiet: if nothing is urgent, nothing is sent.",html:sectionCard('📲','The morning digest','WhatsApp + email, every weekday morning',BLUE)},
  {say:"And a bonus that pairs with all of this: Bulk Nudge. Set a threshold — say, no reply in ten days — and it finds every lead that's gone cold, drafts a friendly follow-up for each, and sends them in one pass. A dozen nudges in the time it used to take to write one.",html:bulletsSlide('Bonus — Bulk Nudge',['Pick a threshold — e.g. no reply in <b>10 days</b>','It finds every cold lead automatically','Drafts a friendly follow-up for each','Sends them all in <b>one pass</b>'],PURP)},
  {say:"Put together, here's the new rhythm. The morning digest tells you who to work. The board ranks them. You open a deal, the brief tells you where it stands and what to say — and you act. Every day, in minutes.",html:flowSlide('Your new daily rhythm',[['📲','Digest','who needs you',BLUE],['🔥','Board','ranked list',AMBER],['🧠','Brief','what to say',K],['✅','Act','close the loop',RED]],'From a full inbox to a short, prioritized list — every morning.')},
  {say:"That's the whole point of the system: stop leads from slipping away, and spend your energy on the ones that need you today. Open the app, and it has already done the reading for you.",html:closeSlide('The payoff','Stop dropping the ball',[['🧠 A brief on every deal',K],['🔥 A ranked to-do list',AMBER],['📲 A morning nudge',BLUE]],'Everything is computed from your real emails and notes — nothing to maintain by hand.')}
]},

'deal-docs':{ brand:'Deal Documents & Meeting Notes', slides:[
  {say:"In this short video we'll cover two new homes for the paperwork that piles up during a deal — a Documents card and a Meeting Notes card — and how all of it follows the customer when they become a client.",html:titleSlide('DEAL DOCUMENTS & MEETING NOTES','Keep the paperwork with the deal','And carry it over when they convert')},
  {say:"Here's the gap we filled. While a lead is still in the pipeline, they send you things — a signed N-D-A, a Process Authority letter, their formula, their label artwork. Until now there was nowhere to put them, so they lived in your inbox and got lost.",html:bulletsSlide('The gap',['Leads send you docs <b>before</b> they are a client','NDAs, Process Authority letters, formulas, labels','Nowhere to store them on the deal','So they lived in the inbox — and got lost'],RED)},
  {say:"Now every saved deal has a Documents card. Pick a type, attach the file, and it is stored securely against that lead.",html:sectionCard('📎','Documents on the deal','Attach it right where the deal lives',K)},
  {say:"You choose from four types with a tap — N-D-A, Process Authority letter, Formula, or Label and artwork — plus an Other catch-all. Each one is filed by type, so you can find it at a glance.",html:principlesSlide('The document types',K,[['🔒','NDA'],['📜','PA letter'],['⚗️','Formula'],['🎨','Label']])},
  {say:"The magic happens when you convert that lead into a client. One button, and everything moves with them — automatically.",html:sectionCard('🚀','Convert to client','One button, and it all moves over',AMBER)},
  {say:"The hand-off is smart. The Process Authority letter drops straight into the client's compliance slot. Label artwork becomes S-K-U entries in their artwork section. And every other document lands on the client's own Documents card. Nothing is copied or lost — the files simply follow the customer.",html:flowSlide('The convert hand-off',[['📜','PA letter','→ compliance slot',AMBER],['🎨','Labels','→ SKU artwork',K],['📎','The rest','→ client docs',BLUE]],'Nothing is re-uploaded — the documents simply follow the customer.')},
  {say:"The second addition is a Meeting Notes card, on every deal and every client.",html:sectionCard('🗒️','Meeting Notes','On every deal and client',PURP)},
  {say:"After a call, paste your notes — or your Pocket A-I NoteTaker transcript — right onto the deal, and attach the recording or file if you have one. And because the Deal Brief reads those notes too, what you discussed on a call shows up in the summary, right alongside the emails.",html:bulletsSlide('How you use it',['Paste notes or your <b>Pocket AI NoteTaker</b> transcript','Attach the file or recording if you have one','The <b>Deal Brief reads them too</b>','Calls and emails, in one summary'],PURP)},
  {say:"Put together, nothing falls through the cracks between the pipeline and the client record. The N-D-A, the formula, the label, the meeting — they start on the deal, and they follow the customer for good.",html:closeSlide('The payoff','Nothing lost between lead and client',[['📎 Docs on the deal',K],['🚀 They move on convert',AMBER],['🗒️ Notes feed the brief',PURP]],'Everything a lead sends you now has a home — and it carries over automatically.')}
]}
};
