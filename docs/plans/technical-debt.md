...
| GL-042 | MEDIUM | `index.html` | `uploadClientDocument()` calls `getPublicUrl()` on `client-docs`, which is a **private** bucket — the resulting `/object/public/` URL returns HTTP 400 (probed 2026-08-29). The value is written to `gmp_documents.file_url` and rendered as a plain `<a href>` in `auditor.html` and `crm-gmp.js`. | Any GMP document uploaded through this path gets a permanently dead download link, including on the page external FDA auditors use. Latent so far: 3 `gmp_documents` rows exist and 0 carry a broken URL, so no data is affected yet. | Same shape as GL-038 — store `file_path` and mint a signed URL at render. The auditor page is anon + inspector token, so it needs a signing route rather than a direct `createSignedUrl` call. | Open |
