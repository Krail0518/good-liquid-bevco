...
| GL-DEF-01 | MEDIUM | `vercel.json` | The CSP allows `'unsafe-inline'` in `script-src`. `index.html` is one ~11k-line inline script plus ~37 external tags, so inline execution is currently load-bearing. | Weakens the main defence-in-depth control against injected script. The escaping rules in CLAUDE.md rule 5 are what actually stand between a lead-supplied string and a staff session. | Needs the inline block extracted (GL-037) or nonce/hash-based CSP before the keyword can be dropped. Recorded here so it is not lost — it was previously tracked only in the audit handoff document. | Open |