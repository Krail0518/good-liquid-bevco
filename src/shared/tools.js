  ...
  function tsForm(existing, parent){
    ...
    ov.querySelector('#gl-ts-save').addEventListener('click', async function(){
      ...
      if(isEdit && String(s.id||'').indexOf('local_') !== 0){
        ...
        var ins = await glCheckedInsert(function(sb){ return sb.from('trade_shows').insert([data]).select().single(); });
        if(!ins.ok){ alert('The trade show was NOT saved — nothing was created.\n' + ins.reason); return; }
        window.glTradeShows.unshift(ins.row);
      }
      saveLocal();
      ov.remove();
      if(parent) parent.remove();
...
(function(){
  ...
  function contentForm(existing){
    ...
    ov.querySelector('#gl-cc-save').addEventListener('click', async function(){
      ...
      // and announce success — a record that existed on one device and nowhere
      // else, never uploaded later. Same rule as tests/checked-inserts.test.cjs.
      if(isEdit && String(p.id||'').indexOf('local_') !== 0){
        if(!window.supa){ alert('Not connected to the database — your changes were NOT saved.'); return; }
        try {
        ...
        var ins = await glCheckedInsert(function(sb){ return sb.from('content_calendar').insert([data]).select().single(); });
        if(!ins.ok){ alert('The post was NOT saved — nothing was created.\n' + ins.reason); return; }
        window.glContent.push(ins.row);
      }
      ...
      b.addEventListener('click', async function(){
        ...
        out.innerHTML =
          '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:10px;font-size:12px">' +
            ...
            '<div style="display:flex;gap:6px">' +
              '<button class="cbtn gl-ar-copy" style="font-size:11px;padding:5px 11px">📋 Copy</button>' +
              (sending && to && typeof window.sendMailgunEmail === 'function'
                ? '<button class="cbtn pri gl-ar-send-btn" style="font-size:11px;padding:5px 11px">📧 Send' + ((client.additionalEmails||[]).length ? ' +Cc ' + (client.additionalEmails||[]).length : '') + '</button>'
        ...
        // Bound here rather than through data-gl-action: the draft, the
        // recipient and the cc list are already in scope, so nothing has to be
        // serialised into an attribute and parsed back out.
        var copyBtn = out.querySelector('.gl-ar-copy');
        ...
        var sendBtn = out.querySelector('.gl-ar-send-btn');
        if(sendBtn) sendBtn.addEventListener('click', async function(){
          sendBtn.disabled = true; sendBtn.textContent = 'Sending…';
          var ok = await window.sendMailgunEmail(to, draft.subject, draft.body,
            { cc: client.additionalEmails || [] });
          sendBtn.disabled = false;
          sendBtn.textContent = ok ? '✓ Sent' : '✗ Failed';
          if(ok && typeof window.glAudit === 'function'){
            window.glAudit('ar_email_sent', invId, {
              days_late: late,
              cc_count: (client.additionalEmails || []).length
            });
          }
        });
      });
